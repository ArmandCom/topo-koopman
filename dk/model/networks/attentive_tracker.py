...
class AttentiveTracker(nn.Module):

    def __init__(self, in_channels, feat_cte_dim, feat_dyn_dim, image_size, n_timesteps, cte_app=False, bs=40):
        super(AttentiveTracker, self).__init__()
        ...
        if image_size[0] == 128:
            ...
            #                nn.BatchNorm2d(256), # TODO: Bias to False.
            #                nn.Conv2d(256, last_hidden_dim, 1)])
            # print('fixat que has fet aixo canalla!!!!!!!!!')
            # exit()
            layers.extend([nn.Conv2d(128, 128, 4, 2, 1), # 8, 8
                           nn.CELU(),
                           nn.BatchNorm2d(128),
    ...
    def forward(self, x, dyn_feat_input=None, block='backbone'):
        ...
        if block == 'dyn_track':
            ...
            if self.cte_app: # if i decide that the appearance will be constant all the time
                ...
                # app_ini = app[:, :, :self.n_timesteps]
                # flat_app = app_ini.flatten(start_dim=0, end_dim=1)
                ...
                app = app[:, :, self.n_timesteps:self.n_timesteps+1].repeat(1, 1, T, 1)
