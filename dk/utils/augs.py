...
class MapTransform(object):
    ...
    def __call__(self, vid):
        ...
        # 140208
        if self.pil_convert:
            # image = Image.fromarray(vid[0,...,0])
            x = np.stack([np.asarray(self.transforms(Image.fromarray(v, 'RGB'))) for v in vid])
            return x
        else:
...
def get_train_transforms(img_size, frame_transforms, frame_aug, patch_size):
    norm_size = torchvision.transforms.Resize((img_size, img_size))

    frame_transform = get_frame_transform(frame_transforms, img_size)
    frame_aug = get_frame_aug(frame_aug, patch_size)
    frame_aug = [frame_aug] if frame_aug != '' else NORM
