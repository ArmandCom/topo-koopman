...
class RecKoopmanModel(BaseModel):
    ...
    def forward(self, input, epoch_iter, test=False):
        ...
        cases.append({"obs": None,
                      ...
                      "pose": f_dyn
                     .reshape(bs, self.n_objects, *f_dyn.shape[1:]),
                      "confi": confi,
                      "shape": shape,
                      "f_cte": f_cte,
                      "T": f_dyn.shape[1],
        ...
        # TODO: WIth bouncing balls just sum them up
        # output["rec"] = torch.sum(out_rec.reshape(bs, self.n_objects, -1, *out_rec.size()[1:]), dim=1)
        # output["pred"] = torch.sum(out_pred.reshape(bs, self.n_objects, -1, *out_rec.size()[1:]), dim=1)
